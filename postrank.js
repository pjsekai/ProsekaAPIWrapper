...
exports.POSTRANK = async(client) => {
	...
	rank_embed.setTitle(`${EVENT_NAME[NowEventId-1].name} | イベントランキング`)
		.setImage(
			`https://sekai-res.dnaroma.eu/file/sekai-assets/home/banner/${EVENT_NAME[NowEventId-1].assetbundleName}_rip/${EVENT_NAME[NowEventId-1].assetbundleName}.webp`
		)
		.addField("残り時間:",
			` **${moment(moment(EVENT_NAME[NowEventId-1].aggregateAt).diff(moment(Date.now()))).add(-1,"days").format("DD日HH時間mm分ss秒")}**
	...
	var _1st = await axios({
		...
		url: process.env.SEKAI_PUBLIC_API_GET_EVENT_LIVE,
		params: {
			rank: "1",
		}
	})
